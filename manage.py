...
def manage():
    store_dao = StoreDAO()
    stores = store_dao.get_stores()  # 데이터베이스에서 가게 목록을 가져옵니다
    return render_template('login/manage.html', stores=stores)  # 템플릿에 'stores'를 전달
...
# 가게 수정
@manage_bp.route('/manage/edit/<int:store_id>', methods=['POST'])
...
@manage_bp.route('/manage/edit_data/<int:store_id>', methods=['GET'])
